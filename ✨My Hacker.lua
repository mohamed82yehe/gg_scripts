...
function SKY_SHOP()
    ...
    while true do
        ...
        -- إذا تم الضغط خارج القائمة (إلغاء)
        if isCancelled then
            return -- يغلق القائمة مؤقتاً
        end
        ...
        if choices then
            for i = 1, #items do
                if choices[i] then
                    ...
                    pcall(function()
                        if code == "MANUAL" then
                            local input = gg.prompt({"أدخل الكود -- عدد منتجات الكشك المغلق:"}, nil, {"text"})
                            if input and input[1] then
                                ...
                                if gg.getResultsCount() > 0 then
                                    ...
                                    gg.toast("⚠️ لم يتم العثور على نتائج")
                                end
                            end
                        elseif code == "ALL" then
                            local allSuccess = true
                            for j = 3, #items - 1 do
                                gg.searchNumber(items[j][2], gg.TYPE_DOUBLE)
                                if gg.getResultsCount() > 0 then
                                    gg.editAll("0", gg.TYPE_DOUBLE)
                                else
                                    allSuccess = false
                            ...
                            gg.toast(allSuccess and "تم تفعيل جميع العناصر" or "⚠️ بعض العناصر لم تُفعّل")
                        else
                            ...
                            if gg.getResultsCount() > 0 then
                                ...
                                gg.toast("⚠️ لم يتم العثور على نتائج لـ: "..items[i][1])
                            end
                        end
                    end)
                    gg.clearResults()
...
-- دالة لاستدعاء القائمة عند الضغط على زر الجيم
